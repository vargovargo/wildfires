...
rcpVar <- "RCP4.5 (emissions peak 2040, stabiliazation by 2100)"
rcp <- rcpMat[rcpVar]
...
modelVar <- "CanESM2 (average)"
model <- modelMat[modelVar]

# Define our function
process_single_nc <- function(model, rcp, pop){
...
test <- process_single_nc(model = model,rcp = rcp, pop = pop)

CAonly <- inner_join(all_fire, locationData) %>%
  gather(6:152, key = "year", value = "hectares") %>% 
  mutate(year = as.integer(as.character(year))) %>% 
           mutate(period = factor(ifelse(year %in% c(1961:1990),"baseline (1961-1990)", 
                                         ifelse(year %in% c(2000:2020), "early (2000-2020)",
